...
-- remnants from luagaurd days, too lazy to remove it all for now so enjoy this
...
local function CoreNotify(Title,Subtitle,Duration)
    ...
    local StylePalette = getrenv().require(CorePackages.Workspace.Packages._Workspace.Style.Style.StylePalette)
...
local function TeleportGrabbable(Instance,TargetPosition,StoreItem, OreOnly)
    if Debug then
...
local function GetStoreItem(Name)
    local Model = nil
...
if isLGPremium and isLGPremium() then
    ...
    Toggles.CFLY:OnChanged(function()
        if Toggles.CFLY.Value == true then
    ...
    OreTeleports:AddToggle('StealOwner', {
        Text = 'Owner Spoof',
        Default = false,
        Tooltip = 'Spoofs ownership of items.',
    })

    Toggles.StealOwner:OnChanged(function(Value)
        if Value then
            CoreNotify("Warning", "This feature is detected and may kick you. Please use this on a throwaway account.", 10)
        end
    end)
    
    OreTeleports:AddToggle('OwnerESP', {
        Text = 'Owner ESP',
        Default = false,
    ...
    OreTeleports:AddToggle('nilStealOwner', {
        Text = 'Owner Spoof [BUYER ONLY]',
        Default = false,
        Tooltip = 'Spoofs ownership of items.',
    })
    
    OreTeleports:AddToggle('nilOwnerESP', {
...
local MiscTab = Tabs.Main:AddRightGroupbox('Miscellaneous')
...
local MoneyTab = Tabs.Money:AddLeftGroupbox('Money Utilities')
